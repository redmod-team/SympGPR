...
res = minimize(nll_transform2, np.array((0, 0, 1)), args = (1e-8, xtrainp, ztrainp.T.flatten(), N), method='L-BFGS-B')
print(res.success)
...
for i in range(0, Ntest):
    plt.plot(np.mod(qmap[:,i], 2*np.pi), 1e-2*pmap[:,i], 'k^', label = 'GP', markersize = 0.5)
...
plt.subplot(1,3,2)
plt.plot(np.mod(yinttest[::32,1],2*np.pi), yinttest[::32,0],  color = 'darkgrey', marker = 'o', linestyle = 'None',  markersize = 0.5)
plt.xlabel(r"$\vartheta$", fontsize = 20)
plt.ylabel(r"$p_\vartheta$", fontsize = 20)
plt.ylim([0.007, 0.06])
plt.tight_layout()
