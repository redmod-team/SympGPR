...
REAL*8, intent(in) :: lx
REAL*8, intent(in) :: ly
kern_num = exp(-0.5d0*(y_a - y_b)**2/ly**2 - 0.5d0*(x_a - x_b)**2/lx**2)
end function
...
REAL*8, intent(in) :: lx
REAL*8, intent(in) :: ly
dkdx_num = 1.0d0*(-x_a + x_b)*exp(0.5d0*(-lx**2*(y_a - y_b)**2 - ly**2*( &
      x_a - x_b)**2)/(lx**2*ly**2))/lx**2
end function
...
REAL*8, intent(in) :: lx
REAL*8, intent(in) :: ly
dkdx0_num = 1.0d0*(x_a - x_b)*exp(0.5d0*(-lx**2*(y_a - y_b)**2 - ly**2*( &
      x_a - x_b)**2)/(lx**2*ly**2))/lx**2
end function
...
REAL*8, intent(in) :: lx
REAL*8, intent(in) :: ly
d2kdxdx0_num = 1.0d0*(lx**2 - (x_a - x_b)**2)*exp(0.5d0*(-lx**2*(y_a - &
      y_b)**2 - ly**2*(x_a - x_b)**2)/(lx**2*ly**2))/lx**4
end function
...
d2kdydy0_num = 1.0d0*(ly**2 - (y_a - y_b)**2)*exp(0.5d0*(-lx**2*(y_a - &
      y_b)**2 - ly**2*(x_a - x_b)**2)/(lx**2*ly**2))/ly**4
end function
...
REAL*8, intent(in) :: lx
REAL*8, intent(in) :: ly
d2kdxdy0_num = -1.0d0*(x_a - x_b)*(y_a - y_b)*exp(-0.5d0*(lx**2*(y_a - &
      y_b)**2 + ly**2*(x_a - x_b)**2)/(lx**2*ly**2))/(lx**2*ly**2)
end function
...
REAL*8, intent(in) :: lx
REAL*8, intent(in) :: ly
d3kdxdx0dy0_num = (lx**2 - 1.0d0*(x_a - x_b)**2)*(y_a - y_b)*exp(-0.5d0* &
      (lx**2*(y_a - y_b)**2 + ly**2*(x_a - x_b)**2)/(lx**2*ly**2))/(lx &
      **4*ly**2)
end function
...
d3kdydy0dy0_num = (3.0d0*ly**2 - (y_a - y_b)**2)*(y_a - y_b)*exp(-0.5d0* &
      (lx**2*(y_a - y_b)**2 + ly**2*(x_a - x_b)**2)/(lx**2*ly**2))/ly** &
      6
end function
...
REAL*8, intent(in) :: lx
REAL*8, intent(in) :: ly
d3kdxdy0dy0_num = (1.0d0*ly**2 - (y_a - y_b)**2)*(x_a - x_b)*exp(-0.5d0* &
      (lx**2*(y_a - y_b)**2 + ly**2*(x_a - x_b)**2)/(lx**2*ly**2))/(lx &
      **2*ly**4)
end function
...
REAL*8, intent(in) :: lx
REAL*8, intent(in) :: ly
dkdlx_num = 1.0d0*(x_a - x_b)**2*exp(-0.5d0*(y_a - y_b)**2/ly**2 - 0.5d0 &
      *(x_a - x_b)**2/lx**2)/lx**3
end function
...
REAL*8, intent(in) :: lx
REAL*8, intent(in) :: ly
d3kdxdx0dlx_num = (-2.0d0*lx**4 + 5.0d0*lx**2*(x_a - x_b)**2 - 1.0d0*( &
      x_a - x_b)**4)*exp(-0.5d0*(lx**2*(y_a - y_b)**2 + ly**2*(x_a - &
      x_b)**2)/(lx**2*ly**2))/lx**7
end function
...
REAL*8, intent(in) :: lx
REAL*8, intent(in) :: ly
d3kdydy0dlx_num = 1.0d0*(ly**2 - (y_a - y_b)**2)*(x_a - x_b)**2*exp( &
      -0.5d0*(lx**2*(y_a - y_b)**2 + ly**2*(x_a - x_b)**2)/(lx**2*ly**2 &
      ))/(lx**3*ly**4)
end function
...
REAL*8, intent(in) :: lx
REAL*8, intent(in) :: ly
d3kdxdy0dlx_num = (2.0d0*lx**2 - 1.0d0*(x_a - x_b)**2)*(x_a - x_b)*(y_a &
      - y_b)*exp(-0.5d0*(lx**2*(y_a - y_b)**2 + ly**2*(x_a - x_b)**2)/( &
      lx**2*ly**2))/(lx**5*ly**2)
end function
...
REAL*8, intent(in) :: lx
REAL*8, intent(in) :: ly
d3kdxdx0dly_num = 1.0d0*(lx**2 - (x_a - x_b)**2)*(y_a - y_b)**2*exp( &
      -0.5d0*(lx**2*(y_a - y_b)**2 + ly**2*(x_a - x_b)**2)/(lx**2*ly**2 &
      ))/(lx**4*ly**3)
end function
...
d3kdydy0dly_num = (-2.0d0*ly**4 + 5.0d0*ly**2*(y_a - y_b)**2 - 1.0d0*( &
      y_a - y_b)**4)*exp(-0.5d0*(lx**2*(y_a - y_b)**2 + ly**2*(x_a - &
      x_b)**2)/(lx**2*ly**2))/ly**7
end function
...
REAL*8, intent(in) :: lx
REAL*8, intent(in) :: ly
d3kdxdy0dly_num = (2.0d0*ly**2 - 1.0d0*(y_a - y_b)**2)*(x_a - x_b)*(y_a &
      - y_b)*exp(-0.5d0*(lx**2*(y_a - y_b)**2 + ly**2*(x_a - x_b)**2)/( &
      lx**2*ly**2))/(lx**2*ly**5)
end function
